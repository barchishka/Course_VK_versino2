...
class VK:
    ...
    def get_albums(self, owner_id):
        getalbums_url = 'https://api.vk.com/method/photos.getAlbums'
        getalbums_params = {
            'owner_id': owner_id,
            'need_system': 1
        }
        ...
        if response.status_code == 200 and 'error' not in response.json():
            ...
            print('Введите номер альбома из которого нужно сохранить фотографии')
            ...
            if album_number.isdecimal():
                if int(album_number) in albums_select_number:
                    ...
                    print('Номер альбома не существует!')
                    sys.exit()
            else:
    ...
    def get_photos(self, owner_id, album_id, photos_count=5):
        getphotos_url = 'https://api.vk.com/method/photos.get'
        getphotos_params = {
            'owner_id': owner_id,
            'album_id': album_id,
            'extended': 1,
        ...
        if response.status_code == 200 and 'error' not in response.json():
            ...
            Photos_data = []
            if photos_count == 0:
                print('Ошибка! В выбранном альбоме нет фотографий! Выберите другой альбом.')
                sys.exit()
            ...
            while getphotos_params['offset'] <= photos_count - 1:
                ...
                response = requests.get(getphotos_url, params={**self.params, **getphotos_params})
            Photos_dict = {}
            print('Сохранено фото в .json:')
            with open('Photos.json', 'w') as file:
                for item in tqdm(sizes_dict):
                    time.sleep(0.1)
                    Photos_dict['file_name'] = item
                    Photos_dict['size'] = sizes_dict[item]
                    Photos_data.append(Photos_dict)
                    Photos_dict = {}
                json.dump(Photos_data, file, ensure_ascii=False, indent=2)
            return urls_dict
        elif 'error' in response.json():
            print(
